...
WHERE ship_city IN ('France', 'Germany', 'Spain')
...
SELECT unit_price FROM products WHERE discontinued <> 1
...
SELECT unit_price FROM products WHERE (discontinued <> 1 AND units_in_stock >= 20)
ORDER BY unit_price LIMIT 1;
SELECT unit_price FROM products WHERE (discontinued <> 1 AND units_in_stock >= 20)
ORDER BY unit_price DESC LIMIT 1
