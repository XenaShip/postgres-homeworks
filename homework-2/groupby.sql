...
WHERE ship_country LIKE 'P%' ORDER BY freight

-- 3. фамилию и телефон сотрудников, у которых в данных отсутствует регион (см таблицу employees)

SELECT customer_id, phone FROM customers WHERE region IS NULL;
...
GROUP BY suppliers.country ORDER BY suppliers.country DESC;
...
SELECT(SUM(freight)) FROM orders WHERE ship_region IS NOT NULL 
